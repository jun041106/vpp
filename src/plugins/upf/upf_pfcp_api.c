		   ...
		   pfcp_failed_rule_id_t * failed_rule_id)
{
  ...
  vec_foreach (qer, create_qer)
  {
    ...
    create->policer.key = (ISSET_BIT (qer->grp.fields, CREATE_QER_QER_CORRELATION_ID)) ?
		   ...
		   pfcp_failed_rule_id_t * failed_rule_id)
{
  ...
  vec_foreach (qer, update_qer)
  {
    ...
    update->policer.key = (ISSET_BIT (qer->grp.fields, UPDATE_QER_QER_CORRELATION_ID)) ?
      qer->qer_correlation_id :
      (u64)(sess - gtm->sessions) << 32 | update->id;
